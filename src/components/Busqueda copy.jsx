...
const Busqueda = () => {
    ...
    const { buscador, handleBuscador, usuarios } = useProyectos()

    const usuariosFiltrados = busqueda === '' ? [] : usuarios.filter(usuarios => usuarios.nombre.toLowerCase().includes(busqueda.toLowerCase()))
        ...
        <Transition.Root show={ buscador } as={Fragment} afterLeave={ () => setBusqueda('')  }>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={ handleBuscador }>
                ...
                <Combobox
                    ...
                    onChange={ (usuario) => (window.location = `/proyectos-admin/${usuario._id}`) }
                >
